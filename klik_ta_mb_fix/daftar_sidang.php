...
<head>
  ...
  $sql_tutup  = $conn->query("SELECT * FROM tb_periode_ta ORDER BY id_periode DESC LIMIT 2;");
  ...
  error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));
  $tanggal_hari_ini = date("Y-m-d");
  $sql7  = $conn->query("SELECT * FROM tb_periode_ta ORDER BY id_periode DESC LIMIT 2;");
                      ...
                      <form name="contact_form" id="contact_form" action="proses_jadwalsidang.php?aksi=daftar" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                          ...
                          <select class="form-control" name="skema" id="skema" required="">
                            ...
                            // Tambahkan pengecekan kondisi jika tanggal hari ini berada di antara periode buka dan tutup
                            while ($data6 = mysqli_fetch_assoc($sql6)) {
                              $periode_sidang = $data6['periode_sidang'];
                              $bulan = date("F", strtotime($data6['buka_sidang']));

                              // Tambahkan kondisi di sini untuk memeriksa apakah tanggal hari ini berada di antara periode buka dan tutup
                              if (($tanggal_hari_ini >= $data6['buka_sidang']) && ($tanggal_hari_ini <= $data6['tutup_sidang'])) {
                                if ($count == 1) {
                            ...
                            if ($count == 0) {
                              echo '<option value="">Pendaftaran Sidang Ditutup</option>';
                            }
                            ?>
                          </select>
                        </div>
