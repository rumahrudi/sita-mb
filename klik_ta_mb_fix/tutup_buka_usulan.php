...
<head>
  ...
  <title>SIMP3M | Tutup Buka Usulan Penelitian / Pengabdian</title>
  <!-- Favicon -->
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Tutup Buka Usulan Penelitian / Pengabdian</li>
      </ol>
    </section>
...
<!-- Input addon -->
 <div class="box box-widget" style="margin-bottom: 10px;background-color: #FDFDFD">
            <div class="box-body">
                        <h4 style="margin-bottom: 0px;margin-top: 0px;" class="pull-left text-primary"><i class="fa fa-info-circle" style="margin-right: 6px;"></i> Tutup Buka Usulan Penelitian / Pengabdian</h4>
        ...
        <!-- End Modal -->
          <div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title">Jadwal Tutup Buka Usulan Penelitian / Pengabdian</h3>
            </div>
              ...
              <thead>
               ...
               <tbody>
                ...
                session_start();
                    ...
                    $sql = $conn->query("SELECT *, DATE_FORMAT(`tanggal_buka`, '%D %M %Y') as `tanggal_buka2`, DATE_FORMAT(`tanggal_tutup`, '%D %M %Y') as `tanggal_tutup2` FROM tb_tutup_buka_usulan");
                      while($data = mysqli_fetch_assoc($sql)){
                        echo '<tr>';
                        echo '<td >'.$data['id_tutup_buka'].'</td>';
                        echo '<td >'.$data['jenis_usulan'].'</td>';
                        echo '<td >'.$data['tanggal_buka2'].'</td>';
                        echo '<td >'.$data['tanggal_tutup2'].'</td>';
                        echo '<td align="center">';
                        echo '
                        <a class="btn btn-primary btn-xs" href="" data-toggle="modal" data-target="#myModal'.$data['id_tutup_buka'].'"><i class="fa fa-edit"></i> EDIT</a>
                        <a class="btn btn-danger btn-xs" onclick="return confirm(\'Anda akan menghapus data. Lanjutkan?\');" href="proses_info.php?aksi=delete&id='.$data['id_tutup_buka'].'" ><i class="fa fa-trash"></i> </a>
                        ';
                        echo '</td>';                                
                        echo '</tr>';
                        $no++;
                        ...
                        echo '
                         <div class="modal fade" id="myModal'.$data['id_tutup_buka'].'" role="dialog">
                            <div class="modal-dialog">
                              <!-- Modal content-->
                              <div class="modal-content">
                                <div class="modal-header">
                                ...
                                <div class="modal-body">
                                <form action="proses_info.php?aksi=update_tutup_buka" method="post">

                                <input type="hidden" id="token" name="token" value="'.$token.'">
                                <input type="hidden" id="id_tutup_buka" name="id_tutup_buka" value="'.$data['id_tutup_buka'].'">

                                  <div class="form-group">
                                    <label for="exampleInputEmail1">Jenis Usulan</label>
                                     <input type="text" class="form-control" name="judul" id="judul" value="'.$data['jenis_usulan'].'" readonly="">
                                  </div>
                                  <div class="form-group">
                                    <label for="exampleInputFile">Pilih Tanggal Buka</label>
                                     <input type="date" class="form-control"  name="tanggal_buka" id="tanggal_buka" required="" value="'.$data['tanggal_buka'].'">
                                  </div>
                                  <div class="form-group">
                                    <label for="exampleInputFile">Pilih Tanggal Tutup</label>
                                     <input type="date" class="form-control"  name="tanggal_tutup" id="tanggal_tutup" required="" value="'.$data['tanggal_tutup'].'">
                                  </div>
