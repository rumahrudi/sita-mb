                ...
                $no=1;
                     $sql = $conn->query("SELECT *, DATE_FORMAT(`tanggal_sidang`, '%D %M %Y') as `tanggal` FROM `tb_jadwal_sidang` WHERE periode_sidang = '$periode'");
                       while($data = mysqli_fetch_assoc($sql)){
                        ...
                        $sql10 = $conn->query("SELECT * FROM tb_anggota_tugas_akhir WHERE id_tugas_akhir = $id_ta ");
                        ...
                        if(empty($data6)){
                          ...
                          echo '<td ><a class="btn btn-success btn-xs" data-toggle="modal" data-target="#myModalReviewer'.$idjadwal.'"  role="button">Lihat Komentar</a>';
                          echo '</td>';
