...
<head>
  ...
  <title>SITA | Approval Prasidang</title>
  <!-- Favicon -->
        ...
        <div class="col-xs-12">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">Approval Pra Sidang</h3>
                ...
                <?php
                    ...
                    $no = 1;
                      while($data = mysqli_fetch_assoc($sql)){
                        ...
                        $sql3  = $conn->query("SELECT * FROM `tb_pra_sidang` WHERE `id_mhs` = $id_mhs ;");
                        while($data3 = mysqli_fetch_assoc($sql3)){
                        $id_laporan = $data3['id_pra_sidang'];
                        ...
                        echo '<td ><a class="btn btn-primary btn-xs" data-toggle="modal" href="'.$data3['demo_aplikasi'].'" target="_blank" role="button">Lihat Demo</a></td>';
                        echo '<td >'.$data3['status'].'</td>';
                       echo '<td>
                       <a class="btn btn-primary btn-xs" data-toggle="modal" data-target="#myModalPraSidang'.$id_laporan.'"  role="button">Proses Prasidang</a>
                        </td>';
                        echo '</tr>';
                        $no++;
                        ...
                        <div id="myModalLaporan'.$id_laporan.'" class="modal fade" role="dialog">
                          <div class="modal-dialog modal-lg">
                            ...
                            <div class="modal-content">
                              <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal">&times;</button>
                                <h4 class="modal-title">Draft Laporan TA2</h4>
                              </div>
                              <div class="modal-body" style="height: 600px">
                                <iframe src="viewer.php?file='.base64_encode($data3['draft_dokumen']).'" frameborder="0" width="100%" height="100%" allowtransparency="true"></iframe> 
                              </div>
                              <div class="modal-footer">
                                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                              </div>
                        ...
                        echo '<div id="myModalPraSidang'.$id_laporan.'" class="modal fade" role="dialog" >
                          <div class="modal-dialog">
                            <!-- Modal content-->
                            <div class="modal-content">
                              <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal">&times;</button>
                                <h4 class="modal-title">Approval Pra Sidang</h4>
                              </div>
                              <div class="modal-body">
                              <form class="form-horizontal" name="drop_list" id="from1" action="proses_ta.php?aksi=update_status_prasidang" method="POST" enctype="multipart/form-data">
                                <input type="hidden" id="token" name="token" value="'.$token.'">
                                <input type="hidden" id="id_pra_sidang" name="id_pra_sidang" value="'.$id_laporan.'">
                                <div class="form-group">
                                <label>Pilih Status Prasidang</label>
                                <select class="form-control" name="status_verifikasi">
                                    <option>'.$data3['status'].'</option>
                                ...
                                <textarea class="form-control" rows="3" name="komentar_laporan" placeholder="Tulis disini jika ada file tertentu yang perlu diupload kembali">'.$data3['catatan_dokumen'].'</textarea>
                                </div>
                                <div class="form-group">
                                <label>Komentar Demo Aplikasi</label>
                                <textarea class="form-control" rows="3" name="komentar_aplikasi" placeholder="Tulis disini jika ada file tertentu yang perlu diupload kembali">'.$data3['catatan_aplikasi'].'</textarea>
                                </div>
