...
<head>
  ...
  <title>SITA | List Jadwal Sidang</title>
  <!-- Favicon -->
        ...
        <!-- End Modal -->
          <div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title">Pilih Periode Sidang TA/PA</h3>
            </div>
                ...
                session_start();
                    ...
                    $sql = $conn->query("SELECT *, DATE_FORMAT(`buka_sidang`, '%D %M %Y') as `tanggal_buka2`, DATE_FORMAT(`tutup_sidang`, '%D %M %Y') as `tanggal_tutup2` FROM tb_periode_ta");
                      while($data = mysqli_fetch_assoc($sql)){
                        ...
                        echo '
                        <a class="btn btn-primary btn-xs" href="index.php?page=penilaian_sidang&periode='.base64_encode($data['periode_sidang']).'" "><i class="fa fa-check"></i> Lihat Jadwal Uji Sidang</a>
                        ';
                        echo '</td>';                                
                        echo '</tr>';
                        $no++;
